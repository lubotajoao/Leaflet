...
You'll notice that Sol is at coordinates `[145,175]` instead of `[175,145]`, and the same happens with the map center. Coordinates in `CRS.Simple` take the form of `[y, x]` instead of `[x, y]`, in the same way Leaflet uses `[lat, lng]` instead if `[lng, lat]`.

The debate about whether `[lng, lat]` or `[lat, lng]` or `[y, x]` or `[x, y]` [is not new, and there is no clear consensus](http://www.macwright.org/lonlat/). This lack of consensus is why Leaflet has a class named `LatLng`.
...
If working with `[y, x]` coordinates with something named `L.LatLng` doesn't make much sense to you, you can easily create wrappers for them:
    ...
    L.xy = function(x, y) {
        if (L.Util.isArray(x)) {    // When doing L.xy([x, y]);
            return L.yx(x[1], x[0]);
        }
        return L.yx(y, x);  // When doing L.xy(x, y);
    };

    var sol = L.xy(175, 145);

The [resulting map](crs-simple-example3.html) looks exactly the same, but the code is a bit more readable.
